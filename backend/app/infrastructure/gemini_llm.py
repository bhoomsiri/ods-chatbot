        ...
        def _produce() -> None:
            ...
            try:
                for chunk in model.generate_content(
                    ...
                    # candidate carries no valid Part — e.g. Gemini stops with
                    # finish_reason SAFETY/RECITATION/OTHER. Skip such chunks so a
                    # partial answer survives instead of the whole stream dying.
                    try:
                        text = chunk.text
                ...
                if finish_reason is not None and fr_name not in ("STOP", "MAX_TOKENS"):
                    logging.getLogger("ods.answer").warning(
                        "gemini answer stream ended abnormally: finish_reason=%s",
                        fr_name,
                    )
            finally:
...
- ถ้าคำถามเจาะจงถึง "หัตถการ/การผ่าตัด/โรคเฉพาะ" (เช่น "ผ่าตัด X แบบ ODS ได้ไหม",
  "ทำ Y ได้หรือไม่", "Z หละ") ให้เพิ่ม sub_query หนึ่งข้อที่เป็น "ชื่อหัตถการ/โรคนั้นล้วน ๆ"
  โดย **ห้ามมีคำว่า ODS/วันเดียวกลับ/โรงพยาบาลโพธาราม** ในข้อนั้น (เช่น
  "การผ่าตัดตกแต่งเยื่อแก้วหู Tympanoplasty") เพื่อให้ค้นเอกสารเฉพาะของหัตถการนั้นเจอ
  ไม่ถูกกลบด้วยเอกสารภาพรวม ODS ทั่วไป
